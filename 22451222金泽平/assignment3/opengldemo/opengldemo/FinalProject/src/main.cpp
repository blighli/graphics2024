...
// Calculate the change in mouse position
    ...
    //// Calculate the forward direction (eye -> lookat)
    //f = lookat.add(eye.scale(-1));
    //f = f.normalized();

    //// Calculate the right vector (cross product of forward and up vectors)
    //r = f.cross(Vec3(0.0f, 1.0f, 0.0f));
    //r = r.normalized();

    //// Calculate the up vector (cross product of right and forward vectors)
    //u = r.cross(f);
    //u = u.normalized();
    ...
    //neye2.x = cos(theta) * neye.x + sin(theta) * neye.z;
    //neye2.y = neye.y;
    //neye2.z = -sin(theta) * neye.x + cos(theta) * neye.z;

    //// Update the eye position based on the new horizontal rotation
    //eye = lookat.add(neye2.scale(-1));

    //// Rotate camera vertically (pitch around the x-axis)
    //theta = angleY;
    //f = neye2.scale(-1);  // Forward direction
    //len = f.norm();
    //f = f.normalized();
    //u = u.normalized();
    //neye = f.scale(cos(theta)).add(u.scale(sin(theta))).scale(len);

    //// Update the eye position based on the vertical rotation
